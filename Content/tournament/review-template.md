...
- **独立可利用度交叉核对**（`cross-check-exploitability.py` → `cross-check-report.md`）：
  一份独立于 CFR 求解器的实现只用锁定 equity 表 + 发布频率表，算出每深度可利用度。
  最坏深度可利用度 `________` BB/手（应 ≈ 0；本批次为 0.00000）。是否 ≈ 0:是 / 否
- 另用一份**公开** Nash push/fold 图表抽样比对（来源、版本、是否公开可核对）:`__________`
