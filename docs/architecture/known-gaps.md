...
## 盲注不顺延，破产的盲注位让整手牌没有盲注

- **内容：** `HandState.init` 按固定偏移 1、2 从按钮取小盲与大盲座位，而 `postBlind` 遇到已折出（破产）的座位就静默返回。于是两个盲注位都破产时，整手牌没有任何盲注被贴出：一个孤注下注被所有人弃掉后取回本金，六个座位筹码变化全为 0。200 种子 × 15 手中出现 25 次。
- **为什么是缺陷：** 真实现金局不会在没有大盲的情况下发牌。座位空缺或破产时盲注顺延给下一个仍有筹码的玩家；只剩一个玩家有筹码时根本不成局。
- **我一度把它写进了规格。** 「零增量的手牌必须解释得通」原本枚举两种形态，我在看到 25 手反例后加了第三种「死盲底池」，等于把引擎缺陷合法化——正是本文件存在要防的事。用户指出后撤回。教训：反例出现时，先问「这个形态本身合法吗」，再问「规格是不是漏了」。
- **修法：** 小盲取按钮后第一个有筹码的座位，大盲取其后第一个；有筹码的座位少于两个时不发牌。已写成需求「每一手都有盲注」的三个场景。
- **代价：** 改动会改变发牌与行动序列，所有黄金记录（Session 逐字节记录与四个对手序列）都要重新生成。这是「发布说明级」的改动，不是测试修补。
- **发现时间：** 2026-08-11
