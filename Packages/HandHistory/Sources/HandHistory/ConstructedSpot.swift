...
public enum ConstructedSpotError: Error, Equatable {
    ...
    /// The two hole cards were not exactly two distinct cards.
    case duplicateCards
...
public struct ConstructedSpot: Hashable, Sendable, Codable {
    ...
    /// must parse, then be exactly two distinct cards, then the stack must be
    /// positive, then the seat must exist at a six-handed table. `tableSize` is
    ...
    public init(
        ...
        guard parsed.count == 2, Set(parsed).count == 2 else {
