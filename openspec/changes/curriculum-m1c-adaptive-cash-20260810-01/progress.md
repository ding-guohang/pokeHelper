...
### 复审：最小防守频率——第一个「策略正确性」层面的机械规则

第二轮人工审核判定仍不通过，核心是一条我完全没想到要查的东西：

CO 面对 BTN 的 7.5BB 3bet 只继续 **23.54%**。BTN 冒 7.5BB 去赢 4BB，纯诈唬的
盈亏平衡弃牌率是 7.5/(7.5+4) = **65.22%**，即防守方必须继续 **34.78%**。
继续 23.54% 意味着 **BTN 用任意两张牌 3bet 都能赚 +1.29BB**。

我独立复算，与审核意见分毫不差（329.6 组合、12.74% 跟注、10.80% 4bet）。

关键在于：**前七条审计规则全部通过**。它们查的是「内容是不是它自己声称的那个东西」，
而这一条查的是「内容作为策略是不是自洽」。这是第一条真正意义上的策略正确性规则，
而它同样是可计算的。

修复后继续率 23.54% → **36.41%**，BTN 任意两张的期望从 +1.29BB 变为 **−0.19BB**。

新增规则 8（最小防守频率），并把规则 6 的阈值从 100bp 收紧到 **10bp**：
10bp 是 `DecisionScorer` 的 excellent 上限，混合策略里的每一支都必须评为 excellent，
否则 App 会告诉用户「你选的这条被范围表认可的线只是次优」。收紧后立刻抓出
混合 RFI 手牌 2 milliBB 的 EV 差（翻前小底池里折合 13bp）——均衡里混合手牌与弃牌
本就无差异，应为 0。

### 审核报告本身也在误导审核人

- 「行动频率 raise 100%」看起来像整段范围的开池率，实际只是 AKo 这一手。
  现在分两行：英雄手牌与其牌类、该手牌的频率、以及整段范围的加权频率。
- 面对加注的节点原本显示「整段范围加权继续率 9.04%」——那是对全部 1326 手算的。
  有意义的分母是**走到这个节点的开池范围**，即 36.41%。现在按节点类型选择分母，
  并直接打印该节点的最小防守频率供对照。
- 「跟注至 5.00BB」误导：5BB 是应补金额。改为「补跟 5.00BB（总投入至 7.50BB）」。
...
- **复练调度**：`TrainingCatalogItem` 新增 `curriculumNodeID`，planner 按节点而非
  能力维度匹配到期复练。两个 fixture 原本把两个字段设成同一个字符串，所以领域测试
  永远看不见调用方违反契约。新增 `RepetitionAppFixture`，其节点 ID 与维度**故意不同**，
  并加了一条 App 层测试断言到期节点排在前面（且不是靠字典序侥幸）。
- **诊断**：接进 `TodayView`，有入口、进度、跳过。
- **内容更新**：`AppDependencies` 构造 `ContentUpdateCoordinator`，启动时调用；
  `BundledOnlyContentSource` 现在返回 nil，端点落地时只换这一个类型。

### B 类：掌握判定的两处设计缺陷 —— 已修

- **从不犯错的用户永远无法掌握**：复练存在的意义是巩固错过的东西，没错过就没什么要
  巩固。节点从未失败时复练要求为 0。
- **单场景节点永不可掌握却计入分母**：迁移要求按节点可用场景数封顶；同时学习页把
  「场景数不足以证明迁移」的节点排除出分母——在一个场景上证明的掌握是记住了一手牌，
  不是学会了这个位置。

### C 类：门禁 —— 已修

- 发布门禁现在校验每个包与其 `.sha256` 是否一致，并要求 `reviewed` 内容必须带
  checksum（缺文件会静默跳过校验）。
- 发现策略包不再按文件名，而是看 JSON 里有没有 manifest。**排除机制按文件名，门禁
  也按文件名的话，改个名字两者会一起失效**——实测过。
- 新增「每个配置必须产出它应有的频道」断言。此前把 `Release.xcconfig` 的频道标记
  删掉，脚本照样绿。
- store 频道不再两条臂都 echo。
- 冻结契约按提交范围比较，不再是工作树对 HEAD（一提交就永远为真）。
- `check-proposal-completeness.sh` 的三条静默通过路径全部堵上：只留标题掏空正文、
  改项目符号样式、把 Modified 段移到文件末尾。逐条实测。

### D 类：审计规则自身的漏洞 —— 已修

- 规则 8 不再能靠删 `facingRaiseTo` 关闭：欠款超过一个大盲却没声明就报错。
  另外校验 `pot > facingRaiseTo`，否则最小防守频率会退化为 0。
- 规则 7 的位置顺序从导出推导，不再硬编码四个座位（原来漏掉 SB）。
- 规则 2 容差 100bp → 25bp（实测漂移 1–6bp）。原来的余量足够塞进「UTG 百分之百开 72o」。
  结论里没有百分比不再静默跳过，改为报错。
- 审计从磁盘枚举全部导出，新增导出无法逃过规则。
- `continuationBasisPoints` 先乘后除，修掉最多 17% 的低估。

### 翻后内容已删除

审计一覆盖到 depth 导出，立刻暴露它有和核心集当初完全相同的缺陷。翻后内容依赖牌面
结构，我没有办法把它做对，而审核已经确认生成内容无法被认证。**继续留着就是明知会
教错还发**。删除 `depth-6max-100bb.json` 与 `UnverifiedStrategyPack.json`。

后果如实记录：M1C 不再交付翻后内容，dogfooding 频道目前与商店发布携带相同资源，
只是频道标记不同。频道机制保留并由门禁强制，未审核内容回来时不必重新发明。

### 一个只有 UI 测试能发现的问题

把 UI 测试纳入门禁后立刻发现：容器上的 `.accessibilityIdentifier` 会**覆盖**子元素的
标识符。诊断入口确实渲染了，但 `开始诊断` 按钮的 identifier 变成了容器的
`today.diagnostic.entry`。是先打印真实界面树才看清的，没有猜。
