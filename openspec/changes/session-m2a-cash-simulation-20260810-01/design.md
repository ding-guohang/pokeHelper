...
**原先的 `.trainable`（1000 分，判据为「命中已安装内容」）已删除，因为它有两处错。** 其一，它永远选不进来：`.bigPot` 恒有 5 手候选、分数 ≥2000，所以只靠内容覆盖入选的手挤不进前五。其二，改用不含手牌类别的覆盖键之后，命中率从 0.25% 升到 37.62%（实测 6000 手），「是否被覆盖」不再有区分度。真正有区分度的是**偏离多少**。

这两个数字都是实测的。我最初写进规格的 0.70% 与 83% 是心算估的却当成实测报出——0.70% 漏算了筹码分桶与面对情形的约束，83% 是「五个位置除以六」的朴素计数。实现方在真实路径上跑出真值并指出了差异。规格里出现未经测量的数字，与本项目一路在防的「绿灯证明不了任何事」是同一类错误。
...
| BTN 未面对下注 | `rfi-btn` | 41.22%（2026-08-11 按抽水由 46.33% 收紧） |
